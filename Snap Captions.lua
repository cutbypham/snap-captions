...
local function ScriptIsInstalled()
    ...
    if platform == "Mac" then
        Scripts = removeFirstTwoItemsFromPath(Scripts)
    end
    local match = script_path:find(Scripts)
    return match ~= nil
end
...
local function CreateDialog(title, message, url, noOk, deleteButt, bin)
    if not url then
        url = ""
    end
    local dialog = disp:AddWindow({
        ID = "SnapCaptionsDialog",
        WindowTitle = "Snap Captions | Message",
    ...
    function dialog.On.Delete.Clicked(ev)
        mediaPool:DeleteFolders({bin})
        dialog:Hide()
        dialog = nil
        disp:ExitLoop()
    end

    function dialog.On.LINK.AnchorClicked(ev)
        bmd.openurl(ev.URL)
    end
...
local function LoadUTF8Module()
    ...
    if not hasUtf8Support and SCRIPT_INSTALLED then
        local dialog = CreateDialog("🛑 Uh Oh!",
            "Resolve Couldn't Load My UTF8 Properly, I need this to caption languages other than english!", UTF8FailedLoad )
        dialog:Show()
        dialog:RecalcLayout()
        utf8 = string
    end
...
local nonWindowsBase = (platform == "Mac" and username.."/Library/Application Support") or (platform == "Linux" and username.."/.local/share") or (platform == "Windows" and app:MapPath('Scripts:/Comp/'))
if not bmd.direxists(nonWindowsBase) then
    error("There was an issue getting the Application Support Folder!")
    return
end

local installFolder = (platform == "Mac" and username.."/Library/Application Support/Snap Captions - Installed Packs") or (platform == "Linux" and username.."/.local/share/Snap Captions - Installed Packs") or (platform == "Windows" and app:MapPath('Scripts:/Comp/') .. "Snap Captions - Installed Packs")
...
local function InstallScript()
    ...
    local target_path = app:MapPath('Scripts:/Comp/')
    ...
    if not success then
        local dialog = CreateDialog("🛑 Uh Oh!",
            "Snap Captions Can't Get Permission To Install Into Resolve! The Housing Crisis Even Affects Digital Plugin's Now 🥺", snapInstallFailed )
        dialog:Show()
        dialog:RecalcLayout()
        return false
    end
    ...
    if not bmd.fileexists(utf8_source) then
        local dialog = CreateDialog("🛑 Uh Oh!",
            "The UTF8 File Is Missing! It should have been in the same folder as Snap Captions to ensure it get's included with Installation.", noUTF8File )
        dialog:Show()
        dialog:RecalcLayout()
        return false
    end
    ...
    local cleanpack_target = installFolder .."/".. extractSnapCaptions(parseFilename(cleanpack_source).Name).."/"..parseFilename(cleanpack_source).FullName
    ...
    if not success then
        local dialog = CreateDialog("🛑 Uh Oh!",
            "🤔 Snap Captions Couldn't Install Some Caption Templates To Get Started! 😱 We have no Caption Templates 😱", CleanFailedInstall )
        dialog:Show()
        dialog:RecalcLayout()
        return false
    end
...
local function filterFilesByName(files)
    local matchingFiles = {}
    if #files > 0 then
        for _, filePath in ipairs(files) do
            if bmd.fileexists(filePath) then
                ...
                if lowerFile:match("^snap captions .*%.drb$") then
                    table.insert(matchingFiles, parseFilename(filePath))
                end
            end
        end
...
local function notInstalled(files)
    local installedPacks = mediaStorage:GetSubFolderList(installFolder)
    local newPacks = {}
    for _, foundPack in ipairs(files) do
        local isInstalled = false
        for _, pack in ipairs(installedPacks) do
            if #pack > 0 then
                local drb = parseFilename(listFiles(pack)[1])
                if foundPack.FullName == drb.FullName then
                    isInstalled = true
                    break
                end
            end
        ...
        if not isInstalled then
            table.insert(newPacks, foundPack)
        end
    end
    return newPacks
end

-- Main function to get new packs
local function getNewPacks()
    -- Retrieve the watch folder path
    local watchFolder = app:GetData("SnapCaptions.WatchFolder")

    -- List all files in the watch folder
    local allFiles = listFiles(watchFolder)

    -- Filter files based on the naming criteria
    local matchingFiles = filterFilesByName(allFiles)

    local newPacks = notInstalled(matchingFiles)

    -- Return the matching files
    if newPacks and #newPacks > 0 then
        return newPacks
    end
    return nil
end

local function setNewWatchFolder()
    local newPath = app:MapPath(app:RequestDir(
        app:GetData("SnapCaptions.WatchFolder"),
        {
            FReqS_Title = 'Choose a Folder to Search In',
        }
    ))
    if newPath then    
        app:SetData("SnapCaptions.WatchFolder", newPath)
        return true
    end
    return false
end

local function addToLib(dialog)
    local addtoLib = disp:AddWindow({
        ID = "AddToLib",
        WindowTitle = "Snap Captions - Add to Library",
        ui:VGroup{
            ID = "root",
            FixedSize = {325, 500},
            ui:VGroup{
                ...
                ui:Label{ Text = "ADD TO LIBRARY", Alignment = { AlignCenter = true }, Weight = 0, StyleSheet = TITLE_CSS }
            },
            ui:HGroup{
                Weight = 0,
                ui:Button{ ID = "tutorial", Text = "Tutorial", Weight = 0.5 },
                ui:Button{ ID = "get_Captions", Text = "Get More Text Packs", Weight = 0.5 },
            },
            ui:Label{ Text = "CAPTION PACKS FOUND", Alignment = { AlignCenter = true }, Weight = 0, StyleSheet = SMALLER_TITLE_CSS },
            ui:VGroup{
                ID = 'uninstalledFound',
                Weight = 0.5,
                ui:VGroup{
                    ID = "uninstalledPacks",
                    Weight = 0.5,
                    ui:Tree{ ID = "drbList", HeaderHidden = true, AlternatingRowColors = true, RootIsDecorated = false, Events = {ItemChanged = true}, Alignment = { AlignCenter = true } }
                },
                ui:Button{ ID = "Move", Text = "▼▼▼ ADD SELECTED PACKS ▼▼▼", Weight = 0, StyleSheet = PRIMARY_ACTION_BUTTON_CSS },
            },
            ui:Label{ Text = "YOUR LIBRARY", Alignment = { AlignCenter = true }, Weight = 0, StyleSheet = SMALLER_TITLE_CSS },
            ui:VGroup{
                ID = 'installedPackImport',
                Weight = 0.5,
                ui:VGroup{
                    ID = "installedPacks",
                    ui:Tree{ ID = "PackList", SelectionMode = false, HeaderHidden = true, AlternatingRowColors = true, RootIsDecorated = false, Alignment = { AlignCenter = true } }
                }
            }
        }
    })

    local winItms = addtoLib:GetItems()

    function addtoLib.On.tutorial.Clicked(ev)
        OpenURL(PackMangagerTUT)
    end

    function addtoLib.On.get_Captions.Clicked(ev)
        OpenURL(GetMorePacks)
    end

    function updatePacks(installedPacks, installPacks)
        _G.packs = {}
        ...
        if installedPacks then
            for i, folder in ipairs(installedPacks) do
                ...
                table.insert(packs, listFiles(folder)[1])
            end
        end

        _G.notinstalledPacks = {}
        winItms.drbList:Clear()
        if installPacks then
            for _, drb in ipairs(installPacks) do
                local itrow = winItms.drbList:NewItem()
                local packName = drb.FullName
                itrow.Text[0] = packName
                itrow.CheckState[0] = false and "Checked" or "Unchecked"
                winItms.drbList:AddTopLevelItem(itrow)
                table.insert(notinstalledPacks, drb)
            end
        end
    end
    ...
    function addtoLib.On.Move.Clicked(ev)
        for row = 0, winItms.drbList:TopLevelItemCount() - 1 do
            local item = winItms.drbList:TopLevelItem(row)
            if item.CheckState[0] == "Checked" then
                local packPath = notinstalledPacks[row+1]
                CopyDRB(packPath.FullPath, installFolder .. "/" .. extractSnapCaptions(packPath.Name) .. "/" .. packPath.FullName)
                table.insert(packs, row+1, packPath)
            end
        end
        success = true
        disp:ExitLoop()
    ...
    local installedPacks = mediaStorage:GetSubFolderList(installFolder)
    local newPacks = getNewPacks()

    updatePacks(installedPacks, newPacks)

    function addtoLib.On.AddToLib.Close(ev)
        success = false
        disp:ExitLoop()
    end
...
local function noPacksFound(dialog)
    local function installPacks()
    
        local yourLibrary = disp:AddWindow({
            ID = "YourLibrary",
            WindowTitle = "Snap Captions - Your Library",
            ui:VGroup{
                ID = "root",
                FixedSize = {325, 500},
                ui:VGroup{
                ...
                ui:VGroup{
                    ...
                    ui:Button{ ID = "Add", Text = "USE SELECTED PACK", Weight = 0.5, StyleSheet = PRIMARY_ACTION_BUTTON_CSS }
                }
            }
        })
        
        local winItms = yourLibrary:GetItems()
        
        function yourLibrary.On.tutorial.Clicked(ev)
            OpenURL(PackMangagerTUT)
        end
        
        winItms.PackList:SetHeaderLabels({"Pack Name"})

        function yourLibrary.On.add_Captions.Clicked(ev)
            local setFolder = setNewWatchFolder()
            if setFolder then
                local dialog = CreateDialog("I’m Looking 👀 For Compatible Captions",
                "You may see Terminal windows, this is okay \n" ..
                "Please Hold...", nil, true )
                dialog:Show()
                dialog:RecalcLayout()
                if setFolder then
                    local newPacks = getNewPacks()
                    if newPacks then
                        ...
                        dialog:Hide()
                        dialog = nil
                        local dialog = CreateDialog("🕵️‍♂️ A Mystery!",
                        "Snap Captions Can't Find Any Compatable Caption Packs",
                        noNewPacks)
                        dialog:RecalcLayout()
                        dialog:Show()
                        disp:RunLoop()
        ...
        function updatePacks(installedPacks)
            _G.packs = {}
            winItms.PackList:Clear()
        ...
        updatePacks(installedPacks)
    
        function yourLibrary.On.PackList.ItemChanged(ev)
            if selectedInstalled.Text[0] ~= ev.item.Text[0] then
                if ev.item.CheckState[0] == "Checked" then
                    selectedInstalled.CheckState[0] = "Unchecked"
                    selectedInstalled = ev.item
                end 
            else
                if ev.item.CheckState[0] == "Unchecked" then
                    selectedInstalled.CheckState[0] = "Checked"
                    selectedInstalled = ev.item
                end 
            end
        end
        
        function yourLibrary.On.Add.Clicked(ev)
            local selPack
            for row = 0, winItms.PackList:TopLevelItemCount() - 1 do
                local item = winItms.PackList:TopLevelItem(row)
                if item.CheckState[0] == "Checked" then
                    selPack = row
                    break
                end
            ...
            if selPack == 0 then
                app:GetResolve():GetProjectManager():GetCurrentProject():GetMediaPool():AddSubFolder(app:GetResolve():GetProjectManager():GetCurrentProject():GetMediaPool():GetRootFolder(), 'Snap Captions')
                disp:ExitLoop()
            else
                local packPath = packs[selPack]
        ...
        function yourLibrary.On.YourLibrary.Close(ev)
            passed = true
            disp:ExitLoop()
        end
...
local function CreateToolWindow()
    ...
    function win.On.import_pack.Clicked(ev)
        ...
        if template_folder then
            ...
            "There’s already a Snap Captions Bin installed, Want me to delete it?", nil, false, true, template_folder)
            dialog:RecalcLayout()
            dialog:Show()
            disp:RunLoop()
            template_folder = nil
    ...
    function loadPreset(presetName)
        ...
        win:Find("remove_periods").CheckState = app:GetData("SnapCaptions.Presets."..presetName..".PunctuationAdvanced.remove_periods")
        win:Find("remove_quotes").CheckState = app:GetData("SnapCaptions.Presets."..presetName..".PunctuationAdvanced.remove_quotes")
        win:Find("remove_question").CheckState = app:GetData("SnapCaptions.Presets."..presetName..".PunctuationAdvanced.remove_question")
        win:Find("remove_colon").CheckState = app:GetData("SnapCaptions.Presets."..presetName..".PunctuationAdvanced.remove_colon")
        win:Find("remove_exclaim").CheckState = app:GetData("SnapCaptions.Presets."..presetName..".PunctuationAdvanced.remove_exclaim")
        win:Find("remove_semicolon").CheckState = app:GetData("SnapCaptions.Presets."..presetName..".PunctuationAdvanced.remove_semicolon")
        win:Find("remove_commas").CheckState = app:GetData("SnapCaptions.Presets."..presetName..".PunctuationAdvanced.remove_commas")
        win:Find("remove_dashes").CheckState = app:GetData("SnapCaptions.Presets."..presetName..".PunctuationAdvanced.remove_dashes")
        win:Find("remove_punctuation").CurrentIndex = app:GetData("SnapCaptions.Presets."..presetName..".RemovePunctuation")
    ...
    function win.On.save_settings.Clicked(ev)
        ...
        app:SetData("SnapCaptions.Presets."..presetName..".RemovePunctuation", win:Find("remove_punctuation").CurrentIndex)
        app:SetData("SnapCaptions.Presets."..presetName..".PunctuationAdvanced.remove_periods", win:Find("remove_periods").CheckState)
        app:SetData("SnapCaptions.Presets."..presetName..".PunctuationAdvanced.remove_quotes", win:Find("remove_quotes").CheckState)
        app:SetData("SnapCaptions.Presets."..presetName..".PunctuationAdvanced.remove_question", win:Find("remove_question").CheckState)
        app:SetData("SnapCaptions.Presets."..presetName..".PunctuationAdvanced.remove_colon", win:Find("remove_colon").CheckState)
        app:SetData("SnapCaptions.Presets."..presetName..".PunctuationAdvanced.remove_exclaim", win:Find("remove_exclaim").CheckState)
        app:SetData("SnapCaptions.Presets."..presetName..".PunctuationAdvanced.remove_semicolon", win:Find("remove_semicolon").CheckState)
        app:SetData("SnapCaptions.Presets."..presetName..".PunctuationAdvanced.remove_commas", win:Find("remove_commas").CheckState)
        app:SetData("SnapCaptions.Presets."..presetName..".PunctuationAdvanced.remove_dashes", win:Find("remove_dashes").CheckState)
    ...
    win:Find('MainTabs'):AddTab("Controls")
    win:Find('MainTabs'):AddTab("Settings")
    ...
    function win.On.install.Clicked(ev)
        ...
        if not template_folder then
            AboveWindowOpen = true
            local dialog = CreateDialog("Getting Your Pack Library",
            "You may see Terminal windows, this is okay \n" ..
            "Please Hold...", nil, true )
            dialog:Show()
            dialog:RecalcLayout()
            local passed = noPacksFound(dialog)
            if not passed then  
                while true do
                    bmd.wait(0.5)
                    for _, subfolder in ipairs(mediaPool:GetRootFolder():GetSubFolderList()) do
